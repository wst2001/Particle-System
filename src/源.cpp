...
unsigned int CreateTexture(int size);
// settings
...
float lightCutOff = 90.0f;
bool appearMouse = false;
...
int main()
{
	...
	// -------------------------
	Shader lightingShader("../GLSL/vertex.glsl", "../GLSL/fragment.glsl");
	Shader lightCubeShader("../GLSL/shader_vertex.glsl", "../GLSL/shader_fragment.glsl");
	...
	// -----------------------------------------------------------------------------
	unsigned int diffuseMap = loadTexture("../resources/textures/window.png");
	unsigned int particleMap = CreateTexture(128);
	// shader configuration
	// --------------------
	lightingShader.use();
	lightingShader.setInt("material.diffuse", 0);

	ParticleGenerator ParticleSystem(lightingShader);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		ImGui::SliderInt("Max Particles", &currentParticles, 100, 10000);
		ImGui::InputFloat3("Light Position", &lightPos.x);
		ImGui::InputFloat3("Light Direction", &lightDir.x);
		ImGui::InputFloat("Light Cutoff", &lightCutOff);
		ImGui::Checkbox("Self Lighting Particles", &SelfLight);
		ImGui::End();
		ImGui::Render();
...
unsigned int CreateTexture(int size) {
	...
	float maxDistance = float(size) * sqrtf(2.0f) / 2.0f;
	...
	for (int y = 0; y < size; y++) {
		for (int x = 0; x < size; x++) {
			...
			imageData[offset + 3] = (unsigned char)(alpha * 255.0f);
